...
public class Unit extends GoodsLocation
    ...
    public int validateCurrentStop() {
        if (tradeRoute == null) {
            ...
            if (stops == null || stops.isEmpty()) {
                currentStop = -1;
            } else if (currentStop < 0 || currentStop >= stops.size()) {
    ...
    public static boolean betterDefender(Unit defender, double defenderPower,
        ...
        return defender == null || (defender.isPerson() && other.isPerson()
		    && !defender.isArmed() && other.isArmed()) || ((!defender.isPerson() || !other.isPerson() || !defender.isArmed() || other.isArmed()) && ((!defender.isDefensiveUnit() && other.isDefensiveUnit()) || ((!defender.isDefensiveUnit() || other.isDefensiveUnit()) && defenderPower < otherPower)));
    }
    ...
    public int getMoveCost(Tile from, Tile target, int ml) {
        ...
        if (isBeached(from) || (cost > ml && (ml + 2 >= getInitialMovesLeft() || cost <= ml + 2
    ...
    public boolean canMoveToHighSeas() {
        return isInEurope() || isAtSea() || (getOwner().canMoveToEurope() && getType().canMoveToHighSeas() && getTile().isDirectlyHighSeasConnected());
    }
    ...
    public boolean hasHighSeasMove() {
        return canMoveToHighSeas() || (hasTile() && getMovesLeft() > 0 && any(getTile().getSurroundingTiles(1, 1),
    ...
    public Location getPathStartLocation() {
        ...
        } else if (isNaval() && ret == null) {
			if (getDestination() == null
			    || getDestination() instanceof Map || !(getDestination() instanceof Settlement)) {
    ...
    public PathNode findIntermediatePort(Location dst) {
        ...
        } else if (dstTile == null || getTile().getContiguity() != dstCont) {
    ...
    public PathNode searchForDanger(final int range, final float threat) {
        ...
        int reverseRange = range * (isNaval()
            ...
            .getMovement() / getType().getMovement();
    ...
    public int getLineOfSight() {
        ...
        Set<Modifier> result = new HashSet<>(getModifiers(Modifier.LINE_OF_SIGHT_BONUS,
                                        unitType, turn));
    ...
    /** {@inheritDoc} -vis: This routine has visibility implications. */
    @Override
    ...
    public boolean setLocation(Location newLocation) {
        ...
        if (location != null && !location.remove(this)) {//-vis
    ...
    /** GoodsLocation Inherits GoodsLocation.addGoods GoodsLocation.removeGoods */
    ...
    /** {@inheritDoc} -vis: This routine can change player visibility. */
    @Override
    ...
    public int checkIntegrity(boolean fix) {
        ...
        if (this.destination != null && ((FreeColGameObject)this.destination).isUninitialized()) {
		    if (fix) {
    ...
    public Set<Ability> getAbilities(String id, FreeColGameObjectType fcgot,
        ...
        final Player owner = getOwner();
        final UnitType unitType = getType();
        Set<Ability> result = new HashSet<>(unitType.getAbilities(id));
    ...
    public Set<Modifier> getModifiers(String id, FreeColGameObjectType fcgot,
        ...
        final Player owner = getOwner();
        final UnitType unitType = getType();
        Set<Modifier> result = new HashSet<>(unitType.getModifiers(id, fcgot, turn));
